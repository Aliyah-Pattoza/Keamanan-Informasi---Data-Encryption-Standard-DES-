...
G = Tk()
G.geometry("920x500")
G.title("DES Algorithm")
G.configure(background="#E0B69E")
...
def Encrypt():
    ...
    if len(initialKey) < 8 or len(initialKey) > 8:
        print("Tolong, masukkan 64 bit atau 8 byte sebagai initial key ")
    else:
        byteKey = initialKey.encode('utf-8')
        hexaKey   = str(byteKey.hex())
        print("Representasi heksadesimal untuk initial key: ")
        print(hexaKey, "\n")
        tempKey   = []
        s         = ""
        ...
        print("Representasi binary untuk initial key: ")
        print(binKey,"\n")
        
        # Memproses bit kunci pada array permutasi yang terdiri dari 56 value
        PC1 = [
              57,   49,    41,   33,    25,    17,    9,
               1,   58,    50,   42,    34,    26,   18,
              10,    2,    59,   51,    43,    35,   27,
        ...
        print("Kunci setelah proses PC-1: ")
        pc1K = s.join(tempPCList)
        print(pc1K,"\n")

        # Geser kunci dalam 16 putaran enciphering
        ...
        # Masukkan 16 sub keys pada PC-2
        PC2 = [
            14,    17,   11,    24,     1,    5,
            3,     28,   15,     6,    21,   10,
            23,    19,   12,     4,    26,    8,
        ...
        print("Ke-16 sub kunci untuk operasi enkripsi adalah: ")
        ...
        if len(initialMsg) < 9 or len(initialMsg) > 9:
            print("Tolong, masukkan 64 bit atau 8 byte sebagai initial message\n")
        else:
...
def Decrypt():
    ...
    if len(initialKey) < 8 or len(initialKey) > 8:
        ...
        print("Kunci setelah PC-1: ")
        pc1K = s.join(tempPCList)
        print(pc1K,"\n")

        # Menggeser kunci dalam 16 putaran enciphering
        Kl0 = pc1K[:28]
        Kr0 = pc1K[-28:]
        ...
        if len(cipherMsg) < 17 or len(cipherMsg) > 17:
            print("Cipher text Anda tidak cocok dengan panjang 64-bit!\n")
        else:
            tempMsg = []
            s = ""
            ...
            for i in range(16):
                    ...
                    if (i == 15):
                        ...
                        print("Plain text ="," ",finalPlain_Text)
                        plainText.insert(INSERT, finalPlain_Text)

#================================= BUAT WIDGET ==============================

Label1 = Label(G, text="Key", font="Calibri", bg="#E0B69E").place(x=60, y=60)
ED_Key = Entry(G, textvariable=mainKey, font="Calibri", width="50").place(x=60, y=100)

Label2 = Label(G, text="The Plain Text", font="Calibri", bg="#E0B69E").place(x=140, y=160)
clearBtn1 = Button(G, text="Clear", width="8", font="Calibri", command=clear1).place(x=180, y=310)

Label3 = Label(G, text="The Cipher Text", font="Calibri", bg="#E0B69E").place(x=560, y=160)
clearBtn2 = Button(G, text="Clear", width="8", font="Calibri", command=clear2).place(x=670, y=310)

encBtn = Button(G, text="Encrypt", width="8", font="Calibri", command=Encrypt).place(x=435, y=205)
decBtn = Button(G, text="Decrypt", width="8", font="Calibri", command=Decrypt).place(x=435, y=250)

plainText = ScrolledText(G, height=5, width=40)
cipherText = ScrolledText(G, height=5, width=40)
